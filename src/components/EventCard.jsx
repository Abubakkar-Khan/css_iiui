    ...
    <article className="card flex flex-col group/card overflow-hidden bg-black h-full border border-border justify-between">
      <div>
        ...
        <div className="p-4 bg-surface text-left">
          <div className="flex flex-wrap gap-2 mb-2">
            ...
            {event.eventType && (
              <span className="text-[6px] font-mono text-muted bg-border px-1.5 py-0.2 uppercase border border-border">{event.eventType}</span>
            )}
          </div>
      ...
      <div className="p-4 pt-0 bg-surface">
        <div className="pt-2 border-t border-border/40">
          <Link href={`/events/${event.id}`} className="text-[8px] font-mono uppercase text-white hover:text-muted transition-colors block text-right font-bold">
            Details →
          </Link>
        </div>
      </div>
