...
export default function BlogSection() {
  ...
  return (
    <section id="compulink" className="section py-12 md:py-16">
      ...
      {/* 50/50 columns on md+, unified height. Tweak --blogH to match your mock */}
      <div className="mt-8 grid gap-6 md:grid-cols-2 items-stretch" style={{ '--blogH': '520px' }}>
        ...
        <article className="card p-0 overflow-hidden md:h-[var(--blogH)] flex flex-col">
          ...
          <div className="p-4 md:p-5 flex-1 flex flex-col min-w-0">
            <div className="text-xs uppercase text-white/70 flex items-center gap-4 clamp-1">
              <span>{featured.date}</span>
              <span>{featured.author}</span>
            </div>
            <h3 className="mt-2 text-2xl font-extrabold leading-tight text-balance clamp-2">
              {featured.title}
            </h3>
            <p className="mt-3 text-white/80 clamp-3">{featured.excerpt}</p>
        ...
        {/* RIGHT: Two horizontal cards stacked; each = 50% height of left */}
        <div className="grid gap-6 md:grid-rows-2 md:h-[var(--blogH)] min-h-0">
          {side.map((p, idx) => (
            <article key={idx} className="card p-0 overflow-hidden flex items-stretch md:h-full min-h-0">
              ...
              {/* text column (let it shrink) */}
              <div className="p-4 md:p-5 flex-1 min-w-0 flex flex-col">
                <div className="text-[11px] uppercase text-white/70 flex items-center gap-4 clamp-1">
                  <span>{p.date}</span>
                  <span>{p.author}</span>
                </div>
                <h3 className="mt-1 text-xl font-extrabold leading-snug clamp-2">{p.title}</h3>
                <p className="mt-2 text-white/80 hidden sm:block clamp-2">{p.excerpt}</p>
