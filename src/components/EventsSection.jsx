...
export default function EventsSection({ items = [] }) {
  ...
  // Compute how many cards fit -> arrows move by "page"
  const getPageStep = () => {
    ...
    if (b) stride = Math.max(1, b.offsetLeft - a.offsetLeft); // includes the gap
  ...
  const goPage = (dir) => {
    ...
    setIndex((v) => {
      const next = Math.min(Math.max(v + dir * step, 0), maxIndex);
      return next;
    });
  };

  // Drag / swipe support + snap to nearest on release
  useEffect(() => {
    const track = trackRef.current; if (!track) return;
  ...
  // Card width (match timeline proportions)
  ...
  return (
    <section id="events" className="section py-12 md:py-16">
      ...
      <div className="relative mt-8">
        ...
        >
          {events.map((e, idx) => (
            <li key={idx} className="snap-start" style={{ scrollMarginLeft: '16px' }}>
              {/* Card — NO BORDER */}
              <article
                className={`card ${CARD_W} rounded-lg p-2 text-left bg-black/40 transition hover:-translate-y-0.5 flex flex-col`}
                style={{ border: 0 }} // ensure no border even if .card sets one
              >
                <img
                  src={e.img}
                  alt="event"
